...
class RegistrarSangria
{
    public function handle(User $usuario, float $valor, ?string $motivo = null): SangriaCaixa
    {
        if ($valor <= 0) {
            throw new InvalidArgumentException('Valor da sangria deve ser maior que zero.');
        }
        ...
        return DB::transaction(function () use ($usuario, $valor, $motivo) {
            ...
            if (! $caixa) {
                throw new InvalidArgumentException('Abra o caixa antes de registrar sangria.');
            }
