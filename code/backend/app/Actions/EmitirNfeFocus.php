...
class EmitirNfeFocus
{
    public function __construct(
        private ClienteFocusNfe $cliente,
        private MontarPayloadNfe $montarPayload,
    ) {}
    ...
    public function handle(NotaNfce $nota): NotaNfce
    {
        ...
        $serie = (int) ($config->serie_nfe ?: 1);

        return Cache::lock('nfe_numero_'.$serie, 30)->block(10, function () use ($nota, $config, $ambiente, $serie) {
            $config = ConfiguracaoFiscal::query()->lockForUpdate()->firstOrFail();
            $numero = (int) $nota->numero ?: (int) $config->proximo_numero_nfe;
            if (! $nota->numero) {
            ...
            $ref = 'nfe_venda_'.$nota->venda_id.'_nota_'.$nota->id;
            ...
            $resposta = $this->cliente->enviarNfe($ref, $payload, $ambiente);
            $dados = $resposta->json() ?? [];
            ...
            if ($this->processando($dados)) {
                $consulta = $this->cliente->consultarNfe($ref, $ambiente);
    ...
    private function persistirRetorno(NotaNfce $nota, array $dados, string $ambiente): NotaNfce
    {
        ...
        if ($autorizada) {
            ...
            if ($xml) {
                $xmlPath = "nfe/{$nota->id}.xml";
                Storage::disk('local')->put($xmlPath, $xml);
            }
            if ($danfe) {
                $danfePath = "nfe/{$nota->id}-danfe.pdf";
    ...
    private function mensagemErro(array $dados, ?string $corpo): string
    {
        ...
        return is_string($msg) && $msg !== '' ? $msg : 'Focus NFe recusou a NF-e.';
    }
}
